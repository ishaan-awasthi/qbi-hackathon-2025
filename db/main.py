...
mutations = [
    "p.Arg31Cys", "p.Gly85Glu", "p.Ile148Thr"
]
...
output_csv_path = "./metrics_data.csv"
...
with open(output_csv_path, mode='w', newline='') as csv_file:
    ...
    header = [
        "Structure_Residue", "Before_Clashes", "After_Clashes", "Before_InterresidualContacts", "After_InterresidualContacts",
        "Before_InterhelicalContacts", "After_InterhelicalContacts", "Before_Hbonds", "After_Hbonds", 
        "Delta_Clashes", "Delta_Interresidual", "Delta_Interhelical", "Delta_Hbonds"
    ]
    writer.writerow(header)

    for key, value in final_data.items():
        before_clashes = value['before']['clashes'][0] if value['before']['clashes'] else 0
        after_clashes = value['after']['clashes'][0] if value['after']['clashes'] else 0
        before_interresidual_contacts = value['before']['interresidual_contacts'][0] if value['before']['interresidual_contacts'] else 0
        after_interresidual_contacts = value['after']['interresidual_contacts'][0] if value['after']['interresidual_contacts'] else 0
        before_interhelical_contacts = value['before']['interhelical_contacts'][0] if value['before']['interhelical_contacts'] else 0
        after_interhelical_contacts = value['after']['interhelical_contacts'][0] if value['after']['interhelical_contacts'] else 0
        before_hbonds = value['before']['hbonds'][0] if value['before']['hbonds'] else 0
        after_hbonds = value['after']['hbonds'][0] if value['after']['hbonds'] else 0
        
        # Calculate deltas (after - before)
        delta_clashes = after_clashes - before_clashes
        delta_interresidual = after_interresidual_contacts - before_interresidual_contacts
        delta_interhelical = after_interhelical_contacts - before_interhelical_contacts
        delta_hbonds = after_hbonds - before_hbonds
        
        # Write the row with the delta values
        row = [
            key, before_clashes, after_clashes, before_interresidual_contacts, after_interresidual_contacts, 
            before_interhelical_contacts, after_interhelical_contacts, before_hbonds, after_hbonds, 
            delta_clashes, delta_interresidual, delta_interhelical, delta_hbonds
        ]
        writer.writerow(row)
